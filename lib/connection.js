...
var FTP = module.exports = function() {
  ...
  this._debug && this._debug('[clear curr request] on init');
...
FTP.prototype.connect = function(options) {
  ...
  if (typeof options !== 'object')
    options = {};
  this.connected = false;
  this.options.reconnect = +options.reconnect || 0;
  ...
  this._parser.on('response', function(code, text) {
    ...
    if (retval === RETVAL.ERR_TEMP || retval === RETVAL.ERR_PERM) {
      self._debug && self._debug('[current command] in parser', self._curReq && self._curReq.cmd);
      if (self._curReq)
    ...
    if (self._curReq && retval !== RETVAL.PRELIM) {
      this._debug && this._debug('[clear curr request] in parser');
      self._curReq = undefined;
      self._lastReq = undefined;
      self._send();
    }
  ...
  function donoop() {
    if (!self._socket || !self._socket.writable)
      ...
      self._debug && self._debug('[set curr request] on donoop');
      self._curReq = noopreq;
      self._lastReq = noopreq;
  ...
  function onconnect() {
    ...
    if (self._secstate) {
      ...
      self._debug && self._debug('[set curr request] on connect');
      self._curReq = {
        cmd: '',
        cb: reentry
      };
    ...
    function reentry(err, text, code) {
      ...
      if (!cmd) {
        ...
        self._debug && self._debug('[clear curr request] on auth');
  ...
  var sockErr = null; // capture socket error

  socket.on('error', onerror);
  function onerror(err) {
    debug && debug('[connection] socket error', err);
    clearTimeout(timer);
    clearTimeout(self._keepalive);
  ...
  socket.once('close', function(had_err) {
    var queue = _.clone(self._queue);
    if (self._lastReq) queue.unshift(self._lastReq);
    var pasvSockets = _.clone(self._pasvSockets);
    ondone();
    if (sockErr && self.options.reconnect > 0) {
      var sErr = sockErr;
      sockErr = null;

      if (!queue.length) {
        debug && debug('[reconnection] last ftp command was not found!');
      };

      debug && debug('[reconnection] started');
      self.options.reconnect--;
      self.removeAllListeners('ready');
      self.connect(self.options);
      self.on('ready', function() {
        setTimeout(() => {
          self._pasvSockets = pasvSockets.concat(self._pasvSockets);
          debug && debug(`new connection tasks in queue ${self._queue.length}`);
          self._queue = queue.concat(self._queue);

          debug && debug(`[reconnection] finished, ${self._queue.length}`,
                         `tasks in queue, ${self._pasvSockets.length}`,
                         `pasv sockets`);

          self._lastReq = undefined;
          this._debug && this._debug('[clear curr request] on reconnect');
          self._curReq = undefined;
          self._send();
        }, 1000);
      });
...
FTP.prototype.get = function(path, zcomp, cb) {
  ...
  this._pasv(function(err, sock) {
    ...
    source.emit = function(ev, arg1) {
      if (ev === 'error') {
        self._debug && self._debug(`[ftp download] socket error ${arg1}`);
        if (!sockerr)
          sockerr = arg1;
        return;
      } else if (ev === 'end' || ev === 'close') {
        self._debug && self._debug(`[ftp download] socket event ${ev}, done ${done}`);
        if (!done) {
          done = true;
          ondone();
        }
    ...
    function ondone() {
      ...
      if (done && lastreply) {
        self._send('MODE S', function() {}, true);
      };

      if (done && !isClosed) {
        isClosed = true;
        process.nextTick(function () {
          self._debug && self._debug("[ftp download] socket is finalized");
          source._emit('end');
          source._emit('close');
        });
      };
...
FTP.prototype._pasv = function(cb) {
  ...
  this._send('PASV', function reentry(err, text) {
    ...
    this._debug && this._debug('[clear curr request] on pasv');
    self._curReq = undefined;
...
FTP.prototype._pasvConnect = function(ip, port, cb) {
  ...
  socket.once('connect', function() {
    ...
    if (self.options.secure === true) {
      var secOpts = _.cloneDeep(self.options.secureOptions);
      secOpts.socket = socket;
      try {
        secOpts.session = self._socket.getSession();
        //socket.removeAllListeners('error');
        socket = tls.connect(secOpts);
      } catch (e) {
        self._debug && self._debug('[ftp download] session error', e);
        socket.emit("error", e);
        return;
      };
    ...
    clearTimeout(timer);
    pasvSocket = socket;
    self._pasvSockets.push(pasvSocket);
    cb(undefined, socket);
  });
  socket.once('error', onerror);
  function onerror(err) {
  ...
  socket.once('close', function(had_err) {
    ...
    var idx = self._pasvSockets.indexOf(pasvSocket);
    if (idx > 0) self._pasvSockets.splice(idx, 1);
    pasvSocket = undefined;
  });
...
FTP.prototype._send = function(cmd, cb, promote) {
  ...
  if (!this._curReq && queueLen && this._socket && this._socket.readable) {
    ...
    this._lastReq = this._curReq;
    this._debug && this._debug('[current command] from queue', this._curReq.cmd);
    if (this._curReq.cmd === 'ABOR') {
      for (var sock of this._pasvSockets) sock.aborting = true;
    };
...
FTP.prototype._reset = function() {
  ...
  this._pasvSockets = [];
  this._feat = undefined;
  this._debug && this._debug('[clear curr request] on reset');
  this._curReq = undefined;
  this._lastReq = undefined;
  this._secstate = undefined;
  clearTimeout(this._keepalive);
