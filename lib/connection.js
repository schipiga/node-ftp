...
FTP.prototype.connect = function(options) {
  ...
  var isSockErr = false; // flag to notify socket that it's closed due to error
  ...
  socket.once('close', function(had_err) {
    ...
    ondone();
    if (isSockErr && self.options.reconnect > 0) {
      debug && debug('*****ftp reconnection on error*****');
      isSockErr = false;
      --self.options.reconnect;
      self.removeAllListeners('ready');
      self.connect(self.options);
      self.on('ready', function() {
        debug && debug('*****ready after reconnect*****');
